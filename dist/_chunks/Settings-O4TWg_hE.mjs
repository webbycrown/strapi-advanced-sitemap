...
import { Flex, Box, Popover, Typography, Button, LinkButton, Link as Link$1, Portal, Alert, Field, SubNav, Badge, Modal, TextInput, DesignSystemProvider, Main, IconButton } from "@strapi/design-system";
...
const { useGetAIUsageQuery, useGetAiTokenQuery: useGetAiTokenQuery$1, useLazyGetAiTokenQuery: useLazyGetAiTokenQuery$1 } = aiService;
...
function CollectionTypeModal({ isOpen, onClose, onSaved, onError, typeToEdit = null, contentTypes = [] }) {
  ...
  const labelIds = useMemo(() => {
    ...
    return {
      ...
      basePath: `collection-type-base-path-label-${unique}`,
      filename: `collection-type-filename-label-${unique}`,
      subPath: `collection-type-sub-path-label-${unique}`,
      pattern: `collection-type-pattern-label-${unique}`,
      priority: `collection-type-priority-label-${unique}`,
      frequency: `collection-type-frequency-label-${unique}`,
      lastModified: `collection-type-last-modified-label-${unique}`
    };
  }, []);
  const formatPathForInput = (value = "") => {
    ...
    if (typeToEdit) {
      const resolvedType = (() => {
        if (typeToEdit.type && typeToEdit.type.includes("::")) {
          return typeToEdit.type;
        }
        const candidate = contentTypes.find((ct) => ct.uid === typeToEdit.type || ct.singularName === typeToEdit.type || ct.displayName === typeToEdit.type);
        return candidate?.uid || typeToEdit.type || "";
      })();
      setForm({
        type: resolvedType,
        basePath: formatPathForInput(typeToEdit.basePath || ""),
        filename: typeToEdit.filename || "",
        subPath: formatPathForInput(typeToEdit.subPath || ""),
        pattern: typeToEdit.pattern || "",
        priority: (typeToEdit.priority ?? "0.5").toString(),
        frequency: typeToEdit.frequency || "monthly",
        lastModified: typeToEdit.lastModified || "false"
      });
    } else {
      setForm({ type: "", basePath: "", filename: "", subPath: "", pattern: "", priority: "0.5", frequency: "monthly", lastModified: "false" });
    }
    setErrorMessage(null);
  }, [typeToEdit, isOpen, contentTypes]);
  const setField = (k2, v2) => setForm((f2) => ({ ...f2, [k2]: v2 }));
  const handleSubmit = async () => {
    const priorityValue = Number(form.priority);
    const trimSlashes2 = (value = "") => value.replace(/^\/+/, "").replace(/\/+$/, "");
    const payload = {
      type: form.type,
      basePath: trimSlashes2(form.basePath || ""),
      filename: trimSlashes2(form.filename || ""),
      subPath: trimSlashes2(form.subPath || ""),
    ...
    try {
      setSubmitting(true);
      setErrorMessage(null);
      await fetchClient.post(`/${PLUGIN_ID}/admin`, payload);
      await onSaved?.();
    } catch (error) {
            ...
            /* @__PURE__ */ jsxs(Box, { marginBottom: 3, children: [
              ...
              /* @__PURE__ */ jsx(
                TextInput,
                {
                  ...
                  max: "1",
                  value: form.priority,
                  onChange: (e2) => setField("priority", e2.target.value),
                  disabled: submitting
                }
              )
            ] }),
...
const Settings = () => {
  ...
  const computeManualSitemapUrl = useCallback(
    (sitemap) => {
      ...
      const basePath = formatBasePathForSave(sitemap.basePath || "");
      const path = [basePath, filename].filter(Boolean).join("/");
      const origin2 = backendOrigin;
      if (!origin2) {
  ...
  const hydrateSitemaps = useCallback(
    (raw = []) => raw.map((sitemap) => ({
      ...
      basePath: formatBasePathForDisplay(typeof sitemap?.basePath === "string" ? sitemap.basePath : ""),
      filename: typeof sitemap?.filename === "string" ? sitemap.filename : "",
      urls: Array.isArray(sitemap?.urls) ? sitemap.urls.map((url) => ({
        id: createId(),
        loc: typeof url?.loc === "string" ? url.loc : "",
        priority: url?.priority === void 0 || url?.priority === null ? "" : String(url.priority)
      })) : []
    })),
  ...
  const toPayload = useCallback(
    (sitemaps) => sitemaps.map(({ id: _id, kind, basePath, filename, urls }) => ({
      ...
      urls: Array.isArray(urls) ? urls.map(({ id: _urlId, loc, priority }) => ({
        loc: typeof loc === "string" ? formatSlugForSave(loc) : "",
  ...
  const contentTypeLookup = useMemo(() => {
    ...
    contentTypes.forEach((ct) => {
      if (ct && ct.uid) {
        map2.set(ct.uid, ct.displayName || ct.singularName || ct.uid);
      }
    });
    return map2;
  }, [contentTypes]);
  const fetchClient = useMemo(() => getFetchClient(), []);
  const adminPath = useCallback((subPath) => `/${PLUGIN_ID}${subPath}`, []);
  const loadCollectionConfigs = useCallback(async () => {
    ...
    try {
      ...
      const sorted = [...results].sort((a2, b2) => {
        ...
        const bDate = b2?.createdAt ? new Date(b2.createdAt).getTime() : null;
        if (aDate !== null && bDate !== null && aDate !== bDate) {
          return aDate - bDate;
        }
        const aId = Number(a2?.id) || 0;
        const bId = Number(b2?.id) || 0;
        return aId - bId;
      });
      setCollectionConfigs(sorted);
    } catch (error) {
      setCollectionConfigs([]);
      const message = error?.response?.data?.error?.message || error?.message || collectionLoadFailedMessage;
  ...
  const loadFrontendSettings = useCallback(async () => {
    ...
    try {
      ...
      const message = error?.response?.data?.error?.message || error?.message || frontendLoadFailedMessage;
      setFrontendSettings({ baseUrl: "" });
      setFrontendFeedback({ type: "error", message });
    } finally {
    ...
    try {
      ...
      setContentTypes(
        collectionTypes.filter((type2) => type2?.uid).map((type2) => ({
          uid: type2.uid,
          singularName: type2.singularName || type2.uid,
          displayName: type2.displayName || type2.uid
        }))
      );
    } catch (error) {
      setContentTypes([]);
  ...
  useEffect(() => {
    ...
    const loadManual = async () => {
      ...
      try {
        ...
        setManualSitemaps(hydrated);
        setLastSavedSignature(JSON.stringify(raw));
      } catch (error) {
        if (!isMounted) return;
        setManualSitemaps([]);
        setLastSavedSignature(JSON.stringify([]));
        setFeedback({ type: "error", message: error?.message || loadFailedMessage });
      } finally {
        if (isMounted) {
          setIsLoading(false);
        }
  ...
  const addManualUrl = useCallback(
    (sitemapId) => {
      setManualSitemaps(
        (current) => current.map(
          (sitemap) => sitemap.id === sitemapId ? { ...sitemap, urls: [...sitemap.urls, makeBlankUrl()] } : sitemap
        )
      );
    },
    [makeBlankUrl]
    ...
    setManualSitemaps(
      (current) => current.map(
        (sitemap) => sitemap.id === sitemapId ? {
          ...sitemap,
          urls: sitemap.urls.map((url) => url.id === urlId ? { ...url, [field]: value } : url)
        } : sitemap
      )
    );
  }, []);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const message = error?.response?.data?.error?.message || error?.message || saveFailedMessage;
  ...
  const handleCollectionDelete = useCallback(
    async (id) => {
      ...
      try {
        ...
        const message = error?.response?.data?.error?.message || error?.message || collectionDeleteFailedMessage;
        setCollectionFeedback({ type: "error", message });
      } finally {
          ...
          children: [
            ...
            /* @__PURE__ */ jsx(Box, { marginTop: 4, display: "grid", gap: 2, style: { maxWidth: "480px" }, children: /* @__PURE__ */ jsx(
              TextInput,
              {
                ...
                onChange: (event) => setFrontendSettings((current) => ({ ...current, baseUrl: event.target.value })),
          ...
          children: [
            ...
            manualSitemaps.map((sitemap, sitemapIndex) => {
              ...
              const resolvedPathHint = (() => {
                const basePathValue = formatBasePathForSave(sitemap.basePath || "");
                const filenameValue = trimSlashes(sitemap.filename || "") || "sitemap.xml";
                const joined = [basePathValue, filenameValue].filter(Boolean).join("/");
                return `/${joined}`;
              })();
              const resolvedUrlPreview = (() => {
                const origin2 = frontendOrigin || (typeof window !== "undefined" ? window.location?.origin?.replace(/\/+$/, "") || "" : "");
                if (!origin2) {
                  return resolvedPathHint;
                }
                return `${origin2.replace(/\/+$/, "")}${resolvedPathHint}`;
              })();
              const basePathLabelId = `manual-sitemap-${sitemap.id}-base-path-label`;
                      ...
                      /* @__PURE__ */ jsxs(Box, { marginRight: 3, children: [
                        ...
                        /* @__PURE__ */ jsx(
                          TextInput,
                          {
                            ...
                            disabled: disableActions,
                            onChange: (event) => updateManualSitemap(sitemap.id, "filename", event.target.value)
                          }
                        )
                      ] }),
                      /* @__PURE__ */ jsxs(Box, { children: [
                    ...
                    /* @__PURE__ */ jsxs(Box, { marginTop: 6, children: [
                      ...
                      sitemap.urls.map((url, urlIndex) => {
                        ...
                        const entryPreviewUrl = (() => {
                          const value = typeof url?.loc === "string" ? url.loc.trim() : "";
                          if (!value) return "";
                          if (/^https?:\/\//i.test(value)) {
                            return value;
                          }
                          const slug = trimSlashes(value);
                          const origin2 = frontendOrigin || backendOrigin;
                          if (!origin2) {
                            return slug ? `/${slug}` : "";
                          }
                                    ...
                                    !isIndexSitemap && /* @__PURE__ */ jsxs(Box, { children: [
                                      ...
                                      /* @__PURE__ */ jsx(
                                        TextInput,
                                        {
                                          ...
                                          value: url.priority,
                                          disabled: disableActions,
                                          onChange: (event) => updateManualUrl(sitemap.id, url.id, "priority", event.target.value)
                                        }
                                      )
                                    ] })
                                  ]
          ...
          children: [
            ...
            collectionConfigs.map((config, index) => {
              ...
              const friendlyPath = (() => {
                ...
                const joined = [basePathSegment, filenameSegment].filter(Boolean).join("/");
                return `/${joined}`;
              })();
              const collectionSitemapUrl = (() => {
                if (typeof window === "undefined") return null;
                const origin2 = window.location?.origin?.replace(/\/+$/, "") || "";
                if (!origin2) return null;
                if (friendlyPath) {
                  return `${origin2}${friendlyPath}`;
                }
